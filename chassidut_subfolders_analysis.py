...
def main():
    chassidut_dir = "txt/Chasidut"
    
    if not os.path.exists(chassidut_dir):
        print(f"Directory '{chassidut_dir}' not found!")
        return
    
    print("ANALYZING CHASSIDUT SUBFOLDERS")
    print("==============================")
    
    # Get all immediate subfolders
    subfolders = get_immediate_subfolders(chassidut_dir)
    
    if not subfolders:
        print("No subfolders found within Chassidut directory.")
        return
    
    print(f"Found {len(subfolders)} subfolders within Chassidut.")
    ...
    for subfolder in tqdm(sorted(subfolders), desc="Analyzing subfolders"):
        subfolder_path = os.path.join(chassidut_dir, subfolder)
        print(f"\nAnalyzing: {subfolder}")
    ...
    # Analyze Chassidut root directory (files directly in Chassidut folder, not in subfolders)
    print("\nAnalyzing files directly in Chassidut folder (not in subfolders)")
    root_files = [os.path.join(chassidut_dir, f) for f in os.listdir(chassidut_dir) 
                 if os.path.isfile(os.path.join(chassidut_dir, f)) and f.endswith('.txt')]
    
    if root_files:
        root_stats = {
            'subfolder': '[Root Chassidut folder]',
            'files': 0,
            'words': 0,
            'size_mb': 0,
            'tokens': 0
    ...
    print("-" * 80)
    print(f"TOTAL CHASSIDUT TOKENS: {grand_total_tokens:,}")
